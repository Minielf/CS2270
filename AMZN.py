...
def format_points(raw_data, smoothed_data):
    ...
    for i in range(len(str_days)):
        points.append(Point(i, raw_data.loc[str_days[i], 'AMZN'], smoothed_data.loc[str_days[i], 'AMZN']))
    return points
...
short_ema = msft.ewm(span=5, adjust=False).mean()
...
# plt.scatter(x,y)
plt.plot(short_ema)
plt.title('Stock price for AMZN')
plt.xlabel('date')
plt.ylabel('price')
plt.show()


# print(total_err(shapeErr(format_target_points(target), format_points(test_1, test_1))))
# print(total_err(shapeErr(format_target_points(target), format_points(test_2, test_2))))
# print(total_err(shapeErr(format_target_points(target), format_points(test_3, test_3))))
# print(total_err(shapeErr(format_target_points(target), format_points(short_ema, msft))))
